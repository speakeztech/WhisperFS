...
type CancellationTests(fixture: WhisperClientFixture) =

    [<Fact>]
    member _.``ProcessAsyncWithCancellation respects cancellation token`` () =
        async {
            // Arrange - Create a fresh client for this test
            match fixture.CreateClient() with
            | Error e ->
                Assert.True(false, $"Failed to create client: {e}")
            | Ok client ->
                use _ = client

                // Create audio samples with tone (30 seconds to ensure it takes time to process)
                let samples = TestData.generateTone 440.0 30000 16000 // 30 seconds of 440Hz tone

                // Create cancellation token that cancels after 50ms (CUDA is very fast)
                use cts = new CancellationTokenSource(50)
                ...
                | Ok transcription ->
                    // Log how long it took - if it's less than 50ms, processing finished before cancellation
                    let processingTime = transcription.ProcessingTime.TotalMilliseconds
                    Assert.True(false, $"Processing completed in {processingTime}ms before cancellation (expected to be cancelled after 50ms)")
        } |> Async.RunSynchronously

    [<Fact>]
    member _.``ProcessFileAsyncWithCancellation respects cancellation token`` () =
        async {
            // Arrange - Create a fresh client for this test
            match fixture.CreateClient() with
            | Error e ->
    ...
    [<Fact(Skip = "Parallel processing on same client causes CUDA resource conflicts")>]
    member _.``Multiple concurrent cancellations are handled correctly`` () =
        async {
            // Arrange - Create a fresh client for this test
            match fixture.CreateClient() with
            | Error e ->
    ...
    [<Fact>]
    member _.``Cancellation token does not affect non-cancellable operations`` () =
        async {
            // Arrange - Create a fresh client for this test
            match fixture.CreateClient() with
            | Error e ->
